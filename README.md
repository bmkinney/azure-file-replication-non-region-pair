...
- [Deploy the demonstration profile](#deploy-the-demonstration-profile)
...
The diagram shows the demonstration profile. The existing-resource profile keeps the same data paths, but it can reuse your storage accounts, VNets, private endpoints, DNS zones, and registry, and it also supports directly peered VNets.

## Design

- No VNet peering or public storage access in the demonstration profile.
...
- The demonstration profile also provisions a regional blob account and private container in each region for application or demonstration use. They aren't part of the Azure Files transfer path.

See [docs/infrastructure-plan.md](docs/infrastructure-plan.md) for topology, permission boundaries, and failover controls.

## Deployment profiles

| | Demonstration profile | Existing-resource profile |
| --- | --- | --- |
...
| Use it to | Evaluate the solution in a subscription without existing resources | Add replication to an environment that already has some of the services |
...
| Path | Contents |
| --- | --- |
| `infra/main.bicep`, `infra/main.bicepparam` | Demonstration profile and its default parameters |
| `infra/existing.bicep`, `infra/existing.example.bicepparam` | Existing-resource profile and an annotated example parameter file |
| `infra/modules/foundation.bicep` | Demonstration network, DNS, endpoints, storage, registry, identities, RBAC, and compute |
...
1. Create a local parameter file that Git ignores, and set `alertEmailAddresses` to a monitored address. Optionally change the regions and resource group names. Don't change `resourceGroupLocation` after the first deployment, because resource group locations are immutable.
...
| Registry | `Disabled` for a registry that the templates created, after `scripts/deploy.ps1` or any deployment with `acrPublicNetworkAccess = 'Disabled'`. `infra/main.bicepparam` sets `Enabled` for the first deployment; step 4 of [Deploy the demonstration profile](#deploy-the-demonstration-profile) changes it. |
...
The storage accounts deny public network access, and the demonstration and new accounts also deny shared key access, so seed and inspect test data from inside the jobs, which have private access and a managed identity. Container Apps can [override a job's template for a single execution](https://learn.microsoft.com/azure/container-apps/jobs#start-a-job-execution-on-demand):
...
For the demonstration profile with a local parameter file, add `-ParametersFile ./infra/main.local.bicepparam`. For the existing-resource profile, also identify the replication resource groups and the deployment region. Pass `-SecondaryResourceGroupName` when you set `secondaryResourceGroupName`:
...
```powershell
# Demonstration profile, with the default parameter file
...
| `tests/test-inventory.ps1` | Runs `inventory.ps1` against canned Azure CLI responses: the demonstration profile, local-endpoint and hub-only layouts, subnet sizes, services set to `new`, resource group layouts, and custom names. | Azure CLI with Bicep, to compile the parameter files |
