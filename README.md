...
| Subscription and rights | All referenced resources are in the deployment subscription. The deploying identity can create resources and role assignments in the replication resource groups, and can assign roles on both storage accounts and the registry; the inventory checks the role assignment rights. For new endpoints, it also needs the [additional rights](#reuse-or-create-each-service). |
...
An operator using `scripts/switch-direction.ps1` needs the same deployment permissions because the script redeploys the template, including the role assignments on the storage accounts and the registry. Grant failover operators these rights, or an eligible role they have activated in a test, before an outage; a switch that fails on them leaves the replication direction unchanged.
...
| --- | --- | --- | --- |
| [`scripts/inventory.ps1`](#inventory-check) | Checks the prerequisites of a parameter file and lists what a deployment would provision | No: `show`, `list`, REST `GET`, Bicep build, and what-if | Reader; what-if also needs deployment permissions |
...
`scripts/inventory.ps1` reports which services already exist, what a deployment would create, and what must change first. It's read-only: it runs only Azure CLI `show`, `list`, and REST `GET` requests, Bicep build, and deployment what-if.
...
The role assignment check matters because what-if and validation can't make it: the nested deployments that create the assignments depend on the job identities' principal IDs, so Azure evaluates them only when the deployment runs, and reports them as `NestedDeploymentShortCircuited` until then. The check reads the signed-in identity's active role assignments. It doesn't evaluate deny assignments or role assignment conditions, and it doesn't see roles that are only eligible through Privileged Identity Management, so activate an eligible role before you run it. If another identity will deploy, run the inventory as that identity.
...
When what-if reports `NestedDeploymentShortCircuited` diagnostics, the script explains them; see [Deployment errors](#deployment-errors). When a deployment fails because the signed-in identity can't create the job identities' role assignments, the script lists each refused scope and the fix. A failed run doesn't start replication: the jobs deploy only after their role assignments, so fix the permissions and rerun the same command.
...
| `tests/test-inventory.ps1` | Runs `inventory.ps1` against canned Azure CLI responses: a greenfield deployment, local-endpoint and hub-only layouts, subnet sizes, services set to `new`, resource group layouts, custom names, and role assignment rights. | Azure CLI with Bicep, to compile the parameter files |
...
| `tests/test-deployment-scripts.ps1` | Runs `deploy.ps1` and `switch-direction.ps1` against a fake Azure CLI: `-WhatIf` changes nothing and leaves no temporary files, Azure CLI errors are reported, refused role assignments are summarized by scope, the two stages open and close registry access, and switches find jobs in one or two resource groups. | Nothing beyond PowerShell 7 |
...
| `AuthorizationFailed`, or `InvalidTemplateDeployment` with `Authorization failed for template resource ... does not have permission to perform action 'Microsoft.Authorization/roleAssignments/write'` | The deploying identity can't assign roles on a storage account, the registry, or a resource group. Validation and what-if don't detect this, because the nested deployments that create the assignments run only during the deployment. | Grant Role Based Access Control Administrator, which can be [limited to the two roles that the deployment assigns](#rbac-requirements), or User Access Administrator or Owner, at the scope in the error or above. If the role is eligible through Privileged Identity Management, activate it. Wait a few minutes, then rerun; the jobs aren't created until the assignments succeed. The [inventory check](#inventory-check) reports this before you deploy. |
| What-if or `deploy.ps1` reports `NestedDeploymentShortCircuited` diagnostics | Nested deployments that use values created during the deployment, such as the job identities' principal IDs, are evaluated only when the deployment runs, so what-if skips their resources. | Nothing; this is expected. What-if doesn't list the skipped resources or check their permissions, so rely on the inventory check for role assignment rights. |
